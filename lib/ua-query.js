...
const getUserAgent = isBrowser() ? navigatorUA : requestUA;
const bind = isBrowser() ? bindNavigator : bindRequest;
...
const RE = {
  crios: /crios\/(\d+)(?!.*edge)/i,
  chrome: /(?:chromium|chrome)\/(\d+)(?!.*edge?)/i,
  firefox: /Firefox\/(\d+)/,
...
function getVersion(regex, req) {
  ...
  const match = RE[regex].exec(getUserAgent(req));
  let version;
...
function check(regex, req, requestedVersion) {
  const version = getVersion(regex, req);
...
function isOld(req, regex, minVersion) {
  const is = check(regex, req, minVersion);
